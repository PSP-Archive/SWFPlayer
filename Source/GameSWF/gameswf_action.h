...
namespace gameswf
{
	...
	struct event_id
	{
			...
			m_key_code((unsigned char) c)
		{
			...
			// Otherwise, don't.
			QASSERT((m_key_code == key::INVALID && (m_id != KEY_PRESS))
	...
	private:
		...
		// to new'd instances.
		action_buffer(const action_buffer& a) { QASSERT(0); }
		void operator=(const action_buffer& a) { QASSERT(0); }
...
# endif
		...
		tu_string* get_mutable_tu_string() { QASSERT(m_type == STRING); return &m_string_value; }
	};
...
// (though I'm not sure).
	...
	struct as_as_function : public ref_counted
	{
		...
		as_as_function(action_buffer* ab, as_environment* env, int start, const array<with_stack_entry>& with_stack)
			...
			m_properties(NULL)
		{
			QASSERT(m_action_buffer);
		}

		void	set_is_function2() { m_is_function2 = true; }
		void	set_local_register_count(uint8 ct) { QASSERT(m_is_function2); m_local_register_count = ct; }
		void	set_function2_flags(uint16 flags) { QASSERT(m_is_function2); m_function2_flags = flags; }

		void	add_arg(int arg_register, const char* name)
		{
			QASSERT(arg_register == 0 || m_is_function2 == true);
			m_args.resize(m_args.size() + 1);
			m_args.back().m_register = arg_register;
			m_args.back().m_name = name;
		}

		void	set_length(int len) { QASSERT(len >= 0); m_length = len; }
	...
	struct as_environment
	{
		...
		void	set_local_frame_top(int t) { QASSERT(t <= m_local_frames.size()); m_local_frames.resize(t); }
