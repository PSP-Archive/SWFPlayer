...
namespace gameswf
{
	extern bool	IsKeyDown( key::code code );
	extern smart_ptr<movie>	g_active_entity;
	...
	void	generate_mouse_button_events(mouse_button_state* ms)
	{
		...
		g_active_entity = topmost_entity;
	}
	...
	struct button_character_instance : public character
	{
			...
			m_mouse_state(UP)
		{
			...
			if ( m_def != NULL )
			{
				...
				LOG_LINE;
				movie_definition_sub *	movie_def = static_cast< movie_definition_sub * >( parent->get_root_movie()->get_movie_definition() );
				...
				for (r = 0; r < r_num; r++)
				{
					...
					LOG_LINE;
					QASSERT(bdef->m_character_def != NULL);
...
#ifdef COMPETITION_HACK
			return this;
#endif	// #ifdef COMPETITION_HACK
		...
		virtual void	on_button_event(event_id event)
		{
			...
			if (m_def->m_sound != NULL)
			{
				...
				if (s != NULL)
				{
					...
					if (bi >= 0)
					{
						...
						if (bs.m_sound_id > 0)
						{
							QASSERT(m_def->m_sound->m_button_sounds[bi].m_sam != NULL);
			...
			else if (event.m_id == event_id::KEY_PRESS)
			{
				BREAK_POINT( "event.m_key_code = %d", event.m_key_code );

				key_code = key_to_swf_code( static_cast< key::code >( event.m_key_code ) );

				if ( key_code == 0 )
				{
					BREAK_POINT( "Failed to find key code %d", event.m_key_code );
					return;
				}
			}
			...
			for (int i = 0; i < m_def->m_button_actions.size(); i++)
			{
				if ( key_press == true )
				{
					const u8	key_condition( m_def->m_button_actions[i].m_conditions >> 9 );

					BREAK_POINT( "key_condition = %d", key_condition );

					if ( key_condition == key_code )
					{
						// Matching action.
				...
				else if (m_def->m_button_actions[i].m_conditions & c)
				{
					// Matching action.
					for (int j = 0; j < m_def->m_button_actions[i].m_actions.size(); j++)
					{
...
// evan : need set_width and set_height function for struct character
	...
	{
		QASSERT(tag_type == 7 || tag_type == 17 || tag_type == 34);
		...
		{
			QASSERT(m_sound == NULL);	// redefinition button sound is error
