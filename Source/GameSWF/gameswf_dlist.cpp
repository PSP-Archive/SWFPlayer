	...
	// of the first object with a larger depth.
	{
		...
		for (;;)
		{
			...
			else
			{
				...
				QASSERT(m_display_object_array[index].m_character->get_depth() == depth);
				QASSERT(index == 0 || m_display_object_array[index - 1].m_character->get_depth() < depth);
				break;
			}
		}
...
//		IF_VERBOSE_DEBUG(log_msg("dl::add(%d, '%s')\n", depth, ch->get_name()));//xxxxx
		...
		QASSERT(index == find_display_index(depth));
...
//		IF_VERBOSE_DEBUG(log_msg("dl::remove(%d)\n", depth));//xxxxx
		...
		QASSERT(get_character(index)->get_depth() == depth);
		...
		if (id != -1)
		{
			...
			QASSERT(index < size);
			QASSERT(get_character(index)->get_depth() == depth);
			QASSERT(get_character(index)->get_id() == id);
		}
