...
//		memcpy(&m_coords[0], coords, coord_count * sizeof(coords[0]));
	...
	{
		LOG_LINE;
		QASSERT(m_coords.size() > 1);
		QASSERT((m_coords.size() & 1) == 0);
	...
	struct tri_stripper
	{
		...
		// Add two triangles to our strip.
		{
			...
			if (l0.bitwise_equal(r0) == false)
			{
				...
				// the active strips.
				QASSERT(m_last_strip_used >= -1 && m_last_strip_used < m_strips.size());
				int i = m_last_strip_used + 1, n = m_strips.size();
				for ( ; i < n; i++)
				{
					array<point>&	str = m_strips[i];
					QASSERT(str.size() >= 3);	// should have at least one tri already.
...
//	void	mesh_set::set_last_frame_rendered(int frame_counter) { m_last_frame_rendered = frame_counter; }
	...
	// Throw our meshes at the renderer.
	{
		...
		for (int i = 0; i < m_meshes.size(); i++)
		{
			...
			{
				log_error( "mesh fill style mismatch" );
				m_meshes[i].display(fills[0], 1.0);
			}
		}
		...
		{for (int i = 0; i < m_line_strips.size(); i++)
		{
			...
			{
				LOG_LINE;
				log_error( "line fill style mismatch" );
				m_line_strips[i].display(line_styles[0], 1.0);
			}
		}}
		LOG_LINE;
	}
	...
	// specified triangle strip.
	{
		LOG_LINE;
		QASSERT(style >= 0);
		QASSERT(style < 10000);	// sanity check
	...
	// Add the specified line strip to our list of things to render.
	{
		LOG_LINE;
		QASSERT(style >= 0);
		QASSERT(style < 1000);	// sanity check
		QASSERT(coords != NULL);
		QASSERT(coord_count > 1);
...
//		This whole routine needs checking through
	...
	// error to low error.
	{
		...
		if (m_cached_meshes.size() > 0)
		{
			...
			for (int i = 0, n = m_cached_meshes.size() - 1; i < n; i++)
			{
				...
				QASSERT(a->get_error_tolerance() > b->get_error_tolerance());
			}
			#endif // not NDEBUG
		}
	}
