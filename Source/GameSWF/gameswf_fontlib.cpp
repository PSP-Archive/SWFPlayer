...
// gameswf::fontlib::set_nominal_glyph_pixel_size()
	...
	void	finish_current_texture(movie_definition_sub* owner)
	{
		...
		for (int i = 0, n = s_pending_glyphs.size(); i < n; i++)
		{
			...
			QASSERT(pgi->m_glyph_index != -1);
			QASSERT(pgi->m_source_font != NULL);
	...
	// software buffer.
	{
		virtual ~draw_into_software_buffer()	{}
		...
		{
			QASSERT(0);	// Shape glyphs should not contain lines.
		}
	};
	...
	// otherwise doesn't munge the array.
	{
		...
		{for (int i = 0, n = glyph_info->size(); i < n; )
		{
			...
			for (;;)
			{
				...
				QASSERT(width < GLYPH_CACHE_TEXTURE_SIZE);
				QASSERT(height < GLYPH_CACHE_TEXTURE_SIZE);
	...
	// Load a stream containing previously-saved font glyph textures.
	{
		...
		for (int b = 0; b < nb; b++)
		{
			...
			owner->add_bitmap_info(bi.get_ptr());
			QASSERT(bi->get_ref_count() == 2);	// one ref for bi, one for the owner.
		}
