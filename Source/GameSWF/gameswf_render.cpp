...
namespace gameswf 
{
	...
	namespace render
	{
		...
		{
			if (s_render_handler) return s_render_handler->create_bitmap_info_rgb(im, resize);
			else return new bogus_bi;
		}
...
// 			}
		...
		{
			if ( s_render_handler)	return s_render_handler->is_on_screen( coords, vertex_count );
			else					return false;
		}
		...
		{
			if (s_render_handler) s_render_handler->draw_mesh_strip( coords, vertex_count, p_bitmap );
		}
