...
namespace zlib_adapter
{
	...
	struct inflater_impl
	{
		...
		// to before they started.
		{
			if (m_zstream.avail_in > 0)
			{
				...
				QASSERT(pos >= 0);
				QASSERT(pos >= m_initial_stream_pos);
				QASSERT(rewound_pos >= 0);
				QASSERT(rewound_pos >= m_initial_stream_pos);
	...
	// Try to go to pos.  Return actual pos.
	{
		...
		while (inf->m_logical_stream_pos < pos)
		{
			...
			QASSERT(to_read_this_time > 0);

			int	bytes_read = inf->inflate_from_stream(temp, to_read_this_time);
			QASSERT(bytes_read <= to_read_this_time);
			if (bytes_read == 0)
			{
		...
		QASSERT(inf->m_logical_stream_pos <= pos);
