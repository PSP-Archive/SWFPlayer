...
public:
	...
	// Basic array access.
	T&	operator[](int index) { QASSERT(index >= 0 && index < m_size); return m_buffer[index]; }
	const T&	operator[](int index) const { QASSERT(index >= 0 && index < m_size); return m_buffer[index]; }
	...
	// Insert the given element at the end of the array.
	{
		...
		QASSERT(&val < &m_buffer[0] || &val > &m_buffer[m_buffer_size]);
	...
	// realloc?
	{
		...
		if (m_buffer_size == 0) {
			...
			QASSERT(m_buffer);	// need to throw (or something) on malloc failure!
		}			
	}
...
public:
		...
		else
		{
			...
			else
			{
				...
				for (;;)
				{
					...
					QASSERT(collided_index >= 0 && collided_index <= m_table->m_size_mask);
				}
	...
	struct const_iterator
	{
		...
		{
			QASSERT(is_end() == false && (m_hash->E(m_index).is_empty() == false));
	...
	struct iterator : public const_iterator
	{
		...
		{
			QASSERT(const_iterator::is_end() == false);
...
private:
	...
	// Find the index of the matching entry.  If no match, then return -1.
	{
		...
		for (;;)
		{
			QASSERT((e->m_hash_value & m_table->m_size_mask) == (hash_value & m_table->m_size_mask));
			...
			QASSERT(! (e->first == key));	// keys are equal, but hash differs!

			// Keep looking through the chain.
			index = e->m_next_in_chain;
			if (index == -1) break;	// end of chain

			QASSERT(index >= 0 && index <= m_table->m_size_mask);
			e = &E(index);

			QASSERT(e->is_empty() == false);
		}
		return -1;
	}
	...
	// actually contain (which will be less than this).
	{
		...
		QASSERT(new_hash.m_table);	// @@ need to throw (or something) on malloc failure!
