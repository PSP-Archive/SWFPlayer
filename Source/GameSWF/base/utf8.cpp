...
u32	utf8::decode_next_unicode_character(const char** utf8_buffer)
{
	u32	uc;
	char	c;
...
#define NEXT_BYTE(shift)						\
	...
	(*utf8_buffer)++;
	if ((c & 0x80) == 0) return (u32) c;	// Conventional 7-bit ASCII.
...
void	utf8::encode_unicode_character(char* buffer, int* index, u32 ucs_character)
{
...
bool	check_equal(const char* utf8_in, const u32* ucs_in)
{
	for (;;)
	{
		u32	next_ucs = *ucs_in++;
		u32	next_ucs_from_utf8 = utf8::decode_next_unicode_character(&utf8_in);
...
int main(int argc, const char* argv[])
{
	// Simple canned test.
	{
		...
		QASSERT(check_equal(test8, test32));
	}
	...
	if (argc > 1)
	{
		...
		for (;;)
		{
			...
			if (c == '\n' || byte_counter >= LINE_SIZE - 2)
			{
				...
				u32*	q = line_buffer_ucs;
				for (;;)
				{
					u32	uc = utf8::decode_next_unicode_character(&p);
					*q++ = uc;

					QASSERT(q < line_buffer_ucs + LINE_SIZE);
					QASSERT(p < line_buffer_utf8 + LINE_SIZE);
...
// This can be useful for debugging.
				...
				QASSERT(check_equal(line_buffer_utf8, line_buffer_ucs));
				QASSERT(check_equal(reencoded_utf8, line_buffer_ucs));
