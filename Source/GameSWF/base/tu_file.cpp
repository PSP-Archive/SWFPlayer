...
static u32 __attribute__( ( aligned( 64 ) ) )	s_ReadBuffer[ READ_BUFFER_SIZE ];
...
/*	int			num_bytes_read( 0 );
	const int	num_chunks( bytes / READ_BUFFER_SIZE );
	const int	remainder( bytes - ( num_chunks * READ_BUFFER_SIZE ) );
	char *		p_dst( (char *)dst );
	for ( int i = 0; i < num_chunks; ++i )	{
		num_bytes_read += fread( s_ReadBuffer, 1, READ_BUFFER_SIZE, (FILE *)appdata );		memcpy( p_dst, s_ReadBuffer, READ_BUFFER_SIZE );		p_dst += READ_BUFFER_SIZE;	}	num_bytes_read += fread( s_ReadBuffer, 1, remainder, (FILE *)appdata );	memcpy( p_dst, s_ReadBuffer, remainder );	return num_bytes_read;*/	return fread( dst, 1, bytes, (FILE *)appdata );}
...
// number of bytes copied.
{
	...
	while (bytes_left)
	{
		...
		QASSERT(write_count <= read_count);
		QASSERT(read_count <= to_copy);
		QASSERT(to_copy <= bytes_left);
