...
{
	QASSERT(vertex_left_test(b, a, c) <= 0);	// check ccw order
...
// Assert validity.
{
	...
	QASSERT(m_leftmost_vert == -1 || sorted_verts[m_leftmost_vert].m_poly_owner == this);
	...
	do
	{
		...
		// Check ownership.
		QASSERT(pvi->m_poly_owner == this);

		// Check leftmost vert.
		QASSERT(m_leftmost_vert == -1
		...
		QASSERT(sorted_verts[v_next].m_prev == vi);
		...
		if (check_consecutive_dupes && v_next != vi)
		{
			...
			// classification.
			QASSERT((pvi->m_v == sorted_verts[v_next].m_v) == false);
		}
	...
	QASSERT(ear_count == m_ear_count);
	QASSERT(vert_count == m_vertex_count);
	QASSERT(found_leftmost || m_leftmost_vert == -1);
	...
	if (m_reflex_point_index)
	{
		...
		QASSERT(check_count == reflex_vert_count);
	}
	...
	if (m_edge_index)
	{
		...
		QASSERT(check_count == vert_count);
	}
...
{
	QASSERT(m_loop == -1 || sorted_verts[m_loop].m_poly_owner != this);	// make sure our verts have been stolen already.

	m_loop = -1;
	m_leftmost_vert = -1;
	m_vertex_count = 0;

	QASSERT(is_valid(sorted_verts));
}
...
int	compare_polys_by_leftmost_vert(const void* a, const void* b)
{
	...
	else
	{
		...
		QASSERT(poly_a->m_leftmost_vert > poly_b->m_leftmost_vert);
...
{
	QASSERT(vert_index >= 0 && vert_index < sorted_verts->size());
	QASSERT(is_valid(*sorted_verts, false /* don't check for consecutive dupes, poly is not finished */));
...
// the edge (v,v1) doesn't intersect any edges in this poly.
{
	...
	QASSERT(pv1->m_poly_owner != this);	// v1 must not be part of this poly already
	...
	for ( ; vi >= 0; vi--)
	{
		...
		QASSERT(compare_vertices<coord_t>((void*) pvi, (void*) pv1) <= 0);
...
// verts being shifted up.
{
	...
	QASSERT(m_reflex_point_index == NULL);
}
...
// Returns true if we clipped any degenerates while looking for ears.
{
	...
	QASSERT(is_valid(*sorted_verts, true /* do check for dupes */));
...
//#define RANDOMIZE
	...
	QASSERT(sorted_verts[next_ear].m_is_ear == true);
...
// (i.e. vertex v1) from this poly.
{
	...
	if (vertex_left_test(pv0->m_v, pv1->m_v, pv2->m_v) == 0)
	{
		...
		QASSERT(0);	// This should have already been removed by remove_degenerate_chain().
	}
	else
	{
	...
	QASSERT(pv0->m_poly_owner == this);
	QASSERT(pv1->m_poly_owner == this);
	QASSERT(pv2->m_poly_owner == this);
...
// Return the index of a vertex just prior to the chain we've removed.
{
	...
	for (;;)
	{
		QASSERT(is_valid(*sorted_verts, false /* don't check for dupes yet */));
	...
	QASSERT(is_valid(*sorted_verts, true /* do check for dupes; there shouldn't be any! */));
...
{
	QASSERT(v_first_in_subloop != v_first_after_subloop);
...
// Insert the edge (vi, vi->m_next) into the index.
{
	...
	QASSERT(m_edge_index);

	// Make sure edge isn't already in the index.
	QASSERT(m_edge_index->find_payload_from_point(sorted_verts[vi].get_index_point(), vi) == NULL);
...
{
	QASSERT(sorted_verts[cone_a_vert].m_v == sorted_verts[cone_b_vert].m_v);
	...
	if (a_in_b_sum >= 4)
	{
		QASSERT(b_in_a_sum <= -2);
		a_in_b = true;
	}
	else if (a_in_b_sum == 3)
	{
		QASSERT(b_in_a_sum <= 3);
...
// and component polys.
{
	...
	QASSERT(m_sorted_verts.size() == 0);
	QASSERT(m_polys.size() == 0);
	...
	for (int i = 0; i < path_count; i++)
	{
		...
		for (int j = 0; j < path_size; j += 2)	// vertex coords come in pairs.
		{
			...
			QASSERT(m_bound.contains_point(ip));
		}
		QASSERT(p->is_valid(m_sorted_verts));
	...
	{for (int i = 0, n = m_polys.size(); i < n; i++)
	{
		...
		QASSERT(m_polys[i]->is_valid(m_sorted_verts));
	}}
}
...
// big continuous poly.
{
	...
	if (m_polys.size() > 1)
	{
		...
		while (m_polys.size() > 1)
		{
			...
			//
			QASSERT(m_sorted_verts[v2].m_poly_owner == m_polys[0]);
			QASSERT(m_sorted_verts[v1].m_poly_owner == m_polys[1]);
	...
	QASSERT(m_polys.size() == 1);
	// QASSERT(all verts in m_sorted_verts have m_polys[0] as their owner);
}
...
{
	QASSERT(vert_on_main_poly != vert_on_sub_poly);
	QASSERT(main_poly != NULL);
	QASSERT(sub_poly != NULL);
	QASSERT(main_poly != sub_poly);
	QASSERT(main_poly == m_sorted_verts[vert_on_main_poly].m_poly_owner);
	QASSERT(sub_poly == m_sorted_verts[vert_on_sub_poly].m_poly_owner);
	...
	QASSERT(pv_main->m_poly_owner->is_valid(m_sorted_verts));
}
...
//#define EMIT_JOINED_POLY
	...
	QASSERT(penv.m_polys.size() == 0);
	// QASSERT(for all penv.m_sorted_verts: owning poly == NULL);

	QASSERT((result->size() % 6) == 0);
}
