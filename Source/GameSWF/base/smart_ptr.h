...
//	void	operator=(const weak_ptr<T>& w);
	T*	operator->() const { QASSERT(m_ptr); return m_ptr; }
...
private:
	// Don't use these.
	weak_proxy(const weak_proxy& w) { QASSERT(0); }
	void	operator=(const weak_proxy& w) { QASSERT(0); }
...
public:
	...
	void	operator=(T* ptr)
	{
		...
		if (m_ptr)
		{
			...
			QASSERT(m_proxy != NULL);
			QASSERT(m_proxy->is_alive());
		}
		else
		{
			m_proxy = NULL;
