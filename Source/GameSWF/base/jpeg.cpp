...
namespace jpeg
{
		...
		// when it needs more data from the file.
		{
			...
			if (bytes_read <= 0) {
				...
				if (src->m_start_of_file) {
					...
					BREAK_POINT( "empty jpeg source stream." );
				}
	...
	struct rw_dest
	{
		...
		// Write the output buffer into the stream.
		{
			...
			if (dest->m_out_stream->write_bytes(dest->m_buffer, IO_BUF_SIZE) != IO_BUF_SIZE)
			{
				...
				BREAK_POINT( "jpeg::rw_dest couldn't write data." );
			}
		...
		// data, and make sure we get deleted.
		{
			...
			if (datacount > 0) {
				...
				{
					// Error.
					BREAK_POINT( "jpeg::rw_dest::term_destination couldn't write data." );
				}
			}
	...
	// object.
	{
		...
		{
			QASSERT(m_compressor_opened == false);
		...
		{
			QASSERT(m_compressor_opened);
			QASSERT(m_cinfo.output_scanline < m_cinfo.output_height);
			int	lines_read = jpeg_read_scanlines(&m_cinfo, &rgb_data, 1);
			QASSERT(lines_read == 1);
