...
// Grid-based container for points.
{
	...
	grid_index_point(const box_t& bound, int x_cells, int y_cells)
		...
		m_y_cells(y_cells)
	{
		QASSERT(x_cells > 0 && y_cells > 0);
		QASSERT(bound.min.x <= bound.max.x);
		QASSERT(bound.min.y <= bound.max.y);
	...
	struct iterator
	{
		...
		// Point at next element in the iteration.
		{
			...
			QASSERT(m_current_cell_x == m_query_cells.min.x);
			QASSERT(m_current_cell_y == m_query_cells.max.y + 1);
		...
		{
			QASSERT(at_end() == false && m_current_entry != NULL);
	...
	iterator	begin(const box_t& q)
	{
		...
		QASSERT(it.m_query_cells.min.x <= it.m_query_cells.max.x);
		QASSERT(it.m_query_cells.min.y <= it.m_query_cells.max.y);
...
private:
	...
	{
		QASSERT(x >= 0 && x < m_x_cells);
		QASSERT(y >= 0 && y < m_y_cells);
	...
	{
		QASSERT(ip.x >= 0 && ip.x < m_x_cells);
		QASSERT(ip.y >= 0 && ip.y < m_y_cells);
	...
	// Removes the entry from the index, and deletes it.
	{
		...
		for (int iy = ib.min.y; iy <= ib.max.y; iy++)
		{
			for (int ix = ib.min.x; ix <= ib.max.x; ix++)
			{
				...
				QASSERT(i < n);	// Didn't find entry in this cell!  Something is wrong.
			}
		}
