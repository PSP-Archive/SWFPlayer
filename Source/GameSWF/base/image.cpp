...
//			dlfree(m_data);
	...
	{
		QASSERT(x >= 0 && x < m_width);
		QASSERT(y >= 0 && y < m_height);
...
//			dlfree(m_data);
	...
	// DELETES image!!!
	{
		QASSERT(image->m_pitch < 65536);	// SDL_Surface only uses Uint16 for pitch!!!
		...
		QASSERT(s->pixels);
		QASSERT(s->format->BytesPerPixel == 3);
		QASSERT(s->format->BitsPerPixel == 24);
