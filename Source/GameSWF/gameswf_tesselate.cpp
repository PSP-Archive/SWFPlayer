...
namespace tesselate
{
	...
	static float	s_tolerance = 50.0f;
	...
	void	begin_shape(trapezoid_accepter* accepter, float curve_error_tolerance)
	{
		...
		QASSERT(s_current_segments.size() == 0);
		s_current_segments.resize(0);

		QASSERT(s_current_path.size() == 0);
		s_current_path.resize(0);

		QASSERT(curve_error_tolerance > 0);
/*		if (curve_error_tolerance > 0)
	...
	// trapezoids.  Modifies the values in s_current_segments.
	{
		...
		for (int i = i0; i < i1; i++)
		{
			...
			QASSERT(f->m_begin.m_y == y0);
			QASSERT(f->m_end.m_y >= y1);
