...
//		BREAK_POINT( "m_mesh->display" );
	...
	void	morph2_character_def::read(stream* in, int tag_type, bool with_style, movie_definition_sub* md)
	{
		...
		BREAK_POINT( "morph2_character_def::read" );
		...
		BREAK_POINT( "fill_style_count = %d", fill_style_count );
		...
		QASSERT(m_shape1->m_fill_styles.size() == m_shape2->m_fill_styles.size());
		QASSERT(m_shape1->m_line_styles.size() == m_shape2->m_line_styles.size());
		...
		QASSERT(edges_count1 == edges_count2);
	}
}
